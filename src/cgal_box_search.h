...
//https://gist.github.com/sloriot/0943fec878401e04930b7bcbd73baf91
    ...
    inline void beam_volumes(
        ...
        for (auto const& x : pair_collisions) {
            ...
            two_rect_from_point_vector_and_zaxis(p0, v0, segment_normal0, type0, polylines_segment_radii[std::get<1>(v)][std::get<2>(v)], volume_length, flip_male, beam_volume[0], beam_volume[1]);
            two_rect_from_point_vector_and_zaxis(p1, v1, segment_normal1, type1, polylines_segment_radii[std::get<3>(v)][std::get<4>(v)], volume_length, flip_male, beam_volume[2], beam_volume[3]);
            ...
            if (type0 + type1 == 0) {
                ...
                for (size_t lid = 0; lid < 2; lid++) {
                    ...
                    bool result = line_plane(s0, cut_plane_, intersection_points[0]);
                    result = line_plane(s1, cut_plane_, intersection_points[1]);
                ...
                IK::Segment_3 s0(beam_volume[0 + shift][0], beam_volume[0 + shift][1]);
                IK::Segment_3 s1(beam_volume[0 + shift][3], beam_volume[0 + shift][2]);
                bool result = line_plane(s0, cut_plane, intersection_points[0]);
                result = line_plane(s1, cut_plane, intersection_points[1]);
                s0 = IK::Segment_3(beam_volume[1 + shift][0], beam_volume[1 + shift][1]);
                s1 = IK::Segment_3(beam_volume[1 + shift][3], beam_volume[1 + shift][2]);
