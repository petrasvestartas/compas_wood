...
def list_polylines_coord(polylines, scale = 10):
    ...
    for polyline in polylines:
        ...
        for point in polyline:
            v.append(point[0]*scale)
            v.append(point[1]*scale)
            v.append(point[2]*scale)
            #v.extend(point)
    _f = (c_size_t * len(f))(*f)
    _v = (c_float * len(v))(*v)
...
class connections_zones(component):
    ...
    def RunScript(self, _data, _join_p, _scale, _extension, _find, _get, _custom_joints, _custom_types):
        # ==============================================================================
        # clear input
        # ==============================================================================
        ...
        ) = list_polylines_coord(_custom_joints,1)
        ...
        lib.ctypes_get_connection_zones_with_custom_joints(
            ...
            #custom joints
            in_joint_polyline_pairs_f,
            byref(in_joint_polyline_pairs_f_s),  # if a value is passed by reference, it must be a pointer
            in_joint_polyline_pairs_v,
            byref(in_joint_polyline_pairs_v_s),  # if a value is passed by reference, it must be a pointer
            #custom joints types
            in_joint_types_v,
            byref(in_joint_types_v_s),
