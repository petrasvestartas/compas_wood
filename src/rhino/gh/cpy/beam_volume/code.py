    ...
    def RunScript(self,
        ...
        class Beam:
            ...
            def __init__(self, id, centrePline, radii, adjustDirection_, planes=None, samples_=20, rebuild=False):
                ...
                point3dArray = None
                self.id = id
                self.centreCurve = Curve.CreateInterpolatedCurve(
                    centrePline, 3, Rhino.Geometry.CurveKnotStyle.ChordPeriodic
                )
                ...
                if planes is not None and len(planes) != 0:
                    ...
                    plane2 = Plane(
                        self.centreCurve.PointAtStart(),
                        plane.ZAxis,
                        Vector3d.CrossProduct(self.centreCurve.TangentAtStart, plane.ZAxis),
                    )
                    plane3 = Plane(
                        self.centreCurve.PointAtEnd(),
                        plane.ZAxis,
                        Vector3d.CrossProduct(self.centreCurve.TangentAtEnd, plane.ZAxis),
                    )
                ...
                if len(radii) == len(centrePline):
                    ...
                    for j in range(len(point3dArray)):
                        ...
                        num2 = MathUtil.RemapNumbers(
                            num1 % 1, 0, 1, radii[int(Math.Floor(num1))], radii[int(Math.Ceiling(num1))]
                        )
            ...
            def CreateBeamCircles(self, rebuild=False):
                return self.centreCurve.CreateBeamCircles(
                    self.radii, self.frames, self.samples, rebuild, self.samples, 1, 1
                )
            ...
            def OrientPlaneTowardsCurveShorterEnd(self, curve, plane, t):
                ...
                copy_curve = copy_curve.Extend(
                    Rhino.Geometry.CurveEnd.Both,
                    copy_curve.GetLength(0.01) * 0.1,
                    Rhino.Geometry.CurveExtensionStyle.Smooth,
                )
            ...
            def CreateOffsetCurve(
                self, centreCurve, radii, frames, samples, rebuild=False, rebuild_pts=20, signX=1, signY=1
            ):
                point3ds = []
                ...
                for i in range(len(numArray)):
                    ...
                    point3ds.append(
                        (plane.Origin + (plane.XAxis * float(signX) * num1)) + (plane.YAxis * float(signY) * num1)
                    )
            ...
            def OffsetCurve(self, rebuild=False):
                return self.CreateOffsetCurve(
                    self.centreCurve, self.radii, self.frames, self.samples, rebuild, self.samples, 1, 1
                )

            def OffsetCurve4(self, rebuild=False):
                return self.CreateOffsetCurve4(
                    self.centreCurve, self.radii, self.frames, self.samples, rebuild, self.samples
                )
        ...
        class Element:
            ...
            def __init__(self, fID, v, p0_, p1_, plGroup, eID=None, key=""):
                ...
                self.outlinesCenter.plane = Plane(
                    (outline.plane.Origin + outline1.plane.Origin) * 0.5, outline.plane.XAxis, outline.plane.YAxis
                )
        ...
        class Elements:
            ...
            def __getitem__(self, index):
                return list(self.eSL.values())[index]
                            ...
                            for j in range(2, len(direction.outlines)):
                                ...
                                if abs(direction.outlines[j].plane.DistanceTo(item.get_From())) >= 0.0001:
                                    ...
                                    if abs(direction.outlines[j].plane.DistanceTo(item.get_To())) >= 0.0001:
                                        continue
                                if (
                                    direction.outlines[0]
                                    .plines[0]
                                    .ClosestPoint(lines[i].get_From())
                                    .DistanceTo(lines[i].get_From())
                                    < 0.001
                                    or direction.outlines[1]
                                    .plines[0]
                                    .ClosestPoint(lines[i].get_From())
                                    .DistanceTo(lines[i].get_From())
                                    < 0.001
                        ...
                        for i in range(len(lines)):
                            if value.Contains(lines[i].get_From()) or not value.Contains(lines[i].get_To()):
                                ...
                                for j in range(2, len(keyValuePair.Value.outlines)):
                                    ...
                                    if keyValuePair.Value.outlinesCenter.plane.get_Origin().DistanceToSquared(
                                        origin + (direction * 1)
                                    ) < keyValuePair.Value.outlinesCenter.plane.get_Origin().DistanceToSquared(
                                        origin - (direction * 1)
                                    ):
                                        direction.Reverse()
            ...
            def SplitCurvesByPlane(self, pline, plane):
                ...
                for curve in curveArray:
                    ...
                    if result:
                        ...
                        if (
                            plane.DistanceTo(curve.PointAtNormalizedLength(0.5)) < 0.01
                            and plane.DistanceTo(curve.PointAtNormalizedLength(0.5)) > -0.01
                        ):
            ...
            def GetNeighbours(self):
                ...
                def SearchCallback(sender, e):
                    ...
                    if plane0.ZAxis.IsParallelTo(plane1.ZAxis, self.parallelTol) == 0:
                        ...
                        fp = fPlane.Origin
                        fXAxis = fPlane.XAxis
                        mPlane = Rhino.Geometry.Plane(
                            mPlane.Origin, Rhino.Geometry.Vector3d.CrossProduct(mPlane.ZAxis, cross), -cross
                        )
                        fPlane = Rhino.Geometry.Plane(
                            fPlane.Origin, Rhino.Geometry.Vector3d.CrossProduct(fPlane.ZAxis, cross), -cross
                        )
                        ...
                        fPlaneClosest = (
                            fPlane2
                            if mPlane1.Origin.DistanceToSquared(fPlane2.Origin)
                            < mPlane1.Origin.DistanceToSquared(fPlane3.Origin)
                            else fPlane3
                        )
                        ...
                        # Create Elements
                        e0 = Element(
                            fID0, Line.Unset, pline0m, pline1m, self.GetPlane(pline0m), None, key0
                        )  # new Line(r0.Center, r1.Center)
                        ...
                        e1 = Element(
                            fID1, Rhino.Geometry.Line.Unset, pline0f, pline1f, self.GetPlane(pline0f), None, key1
                        )
                        e1.beamOrPlate = True
                        e1.d = axis1
                        ...
                        fPlaneUser = self.OrientPlaneTowardsCurveShorterEnd(
                            self.beams[mID].centreCurve.DuplicateCurve(), Plane(bisector), mT, "a"
                        )
                        mPlaneUser = self.OrientPlaneTowardsCurveShorterEnd(
                            self.beams[fID].centreCurve.DuplicateCurve(), Plane(bisector), fT, "b"
                        )
                        ...
                        r1 = Rectangle3d(
                            self.MovePlanebyAxis(mPlaneUser, self.lengthOfBoxes),
                            Interval(-mRadius, mRadius),
                            Interval(-mRadius, mRadius),
                        )
                        ...
                        e0 = Element(
                            fID0,
                            Line(r0.Center, r1.Center),
                            pline0,
                            pline1,
                            self.GetPlane(pline0),
                            None,
                            str(self.beams[a].id) + ";" + str(self.elementCount),
                        )
                        self.elementCount += 1

                        r0 = Rectangle3d(fPlaneUser, Interval(-fRadius, fRadius), Interval(-fRadius, fRadius))
                        r1 = Rectangle3d(
                            self.MovePlanebyAxis(fPlaneUser, self.lengthOfBoxes),
                            Interval(-fRadius, fRadius),
                            Interval(-fRadius, fRadius),
                        )
                        pline0 = Polyline([r0.Corner(0), r0.Corner(1), r1.Corner(1), r1.Corner(0), r0.Corner(0)])
                        pline1 = Polyline([r0.Corner(3), r0.Corner(2), r1.Corner(2), r1.Corner(3), r0.Corner(3)])
                        ...
                        e1 = Element(
                            ...
                            self.GetPlane(pline0),
                            None,
                            str(self.beams[b].id) + ";" + str(self.elementCount),
                        )
                        self.elementCount += 1
        ...
        for i in range(_circles.BranchCount):
            ...
            list_axes_as_curves.append(axis.ToNurbsCurve())
        ...
        indexing = (
            []
        )  # If none is added tree trunks will follow data tree order, for forks it repeat like this  0 0 0 1 1 ...
        ...
        joint_points = (
            []
        )  # If points are given they are used for joint detection, else Curve.ClosestPoints method is used
        cross_lap = (
            _cross_or_toptoside if _cross_or_toptoside is not None else False
        )  # CrossLap - true, Side-End - false
        # print length_of_box
        # optional parameters
        planes = (
            []
        )  # Used only orientation, and most importantly for components such as forks or bent beams with multiple nodes
        ...
        if len(list_axes_as_curves) > 0:
            ...
            if _create_pipes:
                for i in range(_circles.BranchCount):
                    ...
                    brep = Brep.CreateFromLoft(
                        curves, Point3d.Unset, Point3d.Unset, Rhino.Geometry.LoftType.Loose, False
                    )[0]
