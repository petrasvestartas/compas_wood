...
if __name__ == "__main__":
    ...
    parameters: Tuple[str, Any] = [
        ...
        ("SEARCH TYPE", " "), # 0
        ("(0 - side-to-side, 1 - cross, 2 - both)", 0), # 1
        (" ", " "), # 2
        ("OUTPUT TYPE", " "), # 3
        ("(0 - outlines, 1 - lines, 2 - areas, 3 - volumes, 4 - merged joints)     ", wood_globals.output_geometry_type), # 4
        (" ", " "), # 5
        ("SCALE", " "), # 6
        ("x-axis", 1), # 7
        ("y-axis", 1), # 8
        ...
        (" ", " "), # 10
        ("EXTENSION", " "), # 11
        ("x-axis", wood_globals.joint_volume_extension[0]), # 12
        ("y-axis", wood_globals.joint_volume_extension[1]), # 13
        ("z-axis", wood_globals.joint_volume_extension[2]), # 14
        (" ", " "), # 15
        ...
        ("face_to_face_side_to_side_joints_all_treated_as_rotated", wood_globals.face_to_face_side_to_side_joints_all_treated_as_rotated), # 61
        ("face_to_face_side_to_side_joints_rotated_joint_as_average", wood_globals.face_to_face_side_to_side_joints_rotated_joint_as_average), # 62
        ("face_to_face_side_to_side_joints_dihedral_angle", wood_globals.face_to_face_side_to_side_joints_dihedral_angle), # 63
    ...
    values : list[str, float, int, tuple, list, dict, set, bool, None] = [item[1] for item in parameters]  # Second row: values

    # Pass the my_method as the run_method parameter
    form : NamedValuesForm  = NamedValuesForm(names=names, values=values, title="Joinery Parameters", width = 408, height = 600)
