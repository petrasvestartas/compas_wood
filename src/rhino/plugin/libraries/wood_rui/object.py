...
def add_polylines(polylines, case_name):
    """Add a list of polylines to the specified layer and return their GUIDs."""
    
    layer_index = ensure_layer_exists(case_name, "polylines", Color.Red)
    ...
    if "polylines_guid" in wood_rui_globals[case_name]:
        delete_objects(wood_rui_globals[case_name]["polylines_guid"])
    wood_rui_globals[case_name]["polylines_guid"] = polyline_guids
    wood_rui_globals[case_name]["polylines"] = polylines
    
def add_insertion_lines(lines, case_name):
    """Add a list of polylines to the specified layer and return their GUIDs."""
    
    layer_index = ensure_layer_exists(case_name, "insertion", Color.Blue)
    ...
    if "insertion_guid" in wood_rui_globals[case_name]:
        delete_objects(wood_rui_globals[case_name]["insertion_guid"])
    wood_rui_globals[case_name]["insertion_guid"] = line_guids
    wood_rui_globals[case_name]["insertion"] = lines

def add_mesh(mesh, case_name):
    """Add a mesh to the specified layer and return the mesh's GUID."""

    layer_index = ensure_layer_exists(case_name, "mesh")
        
    if wood_rui_globals[case_name]["mesh_guid"] is not None:
        # Replace the mesh
        wood_rui_globals[case_name]["mesh"] = mesh
        Rhino.RhinoDoc.ActiveDoc.Objects.Replace(wood_rui_globals[case_name]["mesh_guid"], mesh)
    else:
        # Add the mesh
        wood_rui_globals[case_name]["mesh"] = mesh
        mesh_guid = Rhino.RhinoDoc.ActiveDoc.Objects.AddMesh(mesh)
        wood_rui_globals[case_name]["mesh_guid"] = mesh_guid
        obj = Rhino.RhinoDoc.ActiveDoc.Objects.Find(mesh_guid)
        obj.Attributes.LayerIndex = layer_index
        obj.CommitChanges()
    
    return wood_rui_globals[case_name]["mesh_guid"]

def add_adjacency(four_indices_face_face_edge_edge, case_name):
    wood_rui_globals[case_name]["adjacency"] = four_indices_face_face_edge_edge

def add_flags(flags, case_name):
    wood_rui_globals[case_name]["flags"] = flags
    
def add_joinery(joinery: List[List[Rhino.Geometry.Polyline]], case_name: str) -> None:
    """
    ...
    :param case_name: The name of the case to be used for layer and group management.
    """
    ...
    layer_index = ensure_layer_exists(case_name, "joinery", Color.Black)
    ...
    for polyline_group in joinery:
        ...
        if group_guids:
            group_name = f"{case_name}_joinery_group"
    ...
    if "joinery_guid" in wood_rui_globals[case_name]:
        delete_objects(wood_rui_globals[case_name]["joinery_guid"])
    
    # Store the new joinery GUIDs and the joinery data in wood_rui_globals
    wood_rui_globals[case_name]["joinery_guid"] = joinery_guids
    wood_rui_globals[case_name]["joinery"] = joinery
    
    # Hide all child layers except the 'joinery' layer
    # hide_non_joinery_layers(plugin_name, case_name, "joinery")