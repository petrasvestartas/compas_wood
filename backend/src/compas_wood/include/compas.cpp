...
namespace compas
{
    ...
    std::tuple<RowMatrixXd, RowMatrixXi> mesh_to_vertices_and_faces(const Mesh &mesh)
    {
        ...
        {
            vertex_matrix(vd, 0) = (double)location[vd][0];
            vertex_matrix(vd, 1) = (double)location[vd][1];
            vertex_matrix(vd, 2) = (double)location[vd][2];
        }
    ...
    ResultMesh result_from_mesh(const Mesh &mesh)
    {
        ...
        for (Mesh::Vertex_index vd : mesh.vertices())
        {
            result_mesh.vertices(vd, 0) = (double)location[vd][0];
            result_mesh.vertices(vd, 1) = (double)location[vd][1];
            result_mesh.vertices(vd, 2) = (double)location[vd][2];
        }
    ...
    std::vector<RowMatrixXd> read_xml_as_polyline_pairs(std::string &filname)
    {
    }
}