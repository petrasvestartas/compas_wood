...
viewer = App(
    show_grid=False, width=3840, height=2160 - 250, enable_sidebar=True, version="120"
)
...
def display_polylines_as_mesh(viewer, polylines, type, scale=0.01):
    ...
    for i in range(len(outPolylines)):
        ...
        if len(outPolylines[i]) > 0:
            viewer.add(
                Collection(outPolylines[i]), color=outPolylinesColors[i], opacity=0.75
            )
...
def test_rtree(selected_id):
    ...
    if boxes_AABB_or_boxes_OOBB is False:
    ...
    if input is not None:
        display_polylines(
            viewer, input, scale, 1.0, 0.0, 0.0, 1, True, 0
        )  # polylines without joints

    if boxes_selected is not None:
        for i in range(len(boxes_selected)):
            if i == 0 and color_first:
                display_mesh(
                    viewer, boxes_selected[i], scale, 206 / 255, 0, 88 / 255.0, 3
                )
            else:
